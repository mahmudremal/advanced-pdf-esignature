...
class Restapi {
	...
	public function rest_api_init() {
		register_rest_route( 'esign/project/rest', '/userserver/(?P<id>\d+)', [
			'methods' => 'POST',
			'callback' => [ $this, 'createuser' ],
		] );
		register_rest_route( 'esign/project/rest', '/system/(?P<object>[a-zA-Z0-9-]+)', [
			'methods' => 'GET',
	...
	public function createuser( $data ) {
		...
		// $userMeta = (object) apply_filters( 'esign/project/usermeta/defaults', [] );
		// foreach( $userMeta as $meta_key => $meta_value ) {
		// 	if( ! isset( $request[ $meta_key ] ) ) {
		// 		$restData[ $meta_key ] = $meta_value;
		// 	}
		// }
		$restResponse = __( 'Essential information missing to proceed request. Try with atleast email and username.', 'esignbinding' );
		if( isset( $request[ 'email' ] ) && ! empty( $request[ 'email' ] ) ) {
			$except = [ 'display_name' ];
			$is_update_allowed = apply_filters( 'esign/project/system/isactive', 'rest-updateprofile' );
			$is_create_allowed = apply_filters( 'esign/project/system/isactive', 'rest-createprofile' );
			$is_email_prevent = apply_filters( 'esign/project/system/isactive', 'rest-preventemail' );
			$is_default_pass = apply_filters( 'esign/project/system/getoption', 'rest-defaultpass', time() );
			$userInfo = get_user_by_email( $request[ 'email' ] );
			$userMeta = array_map( function( $a ){ return $a[0]; }, (array) get_user_meta( $userInfo->ID ) );
			$userMeta = apply_filters( 'esign/project/usermeta/defaults', $userMeta );
			foreach( $userMeta as $meta_key => $meta_value ) {
			...
			if ( ! is_wp_error( $is_executed ) ) {
				if( $is_executed ) {
					$restResponse = ( $is_create_allowed ) ? sprintf( __( 'New User created: %s', 'esignbinding' ), $is_executed) : __( 'Account not matched and no permission to create one.', 'esignbinding' );
				} else {
					$restResponse = __( 'En Existing user profile information matched and nothing happend with them.', 'esignbinding' );
				}
			} else {
				$restResponse = $is_executed->get_error_message();
				if( empty( $restResponse ) ) {
					$restResponse = ( $is_update_allowed ) ? __( 'En Existing user profile information updated.', 'esignbinding' ) : __( 'En Existing user profile information matched and nothing happend with them.', 'esignbinding' );
				}
			}
		}
	...
	public function systemObject( $request ) {
		if( isset( $request[ 'object' ] ) ) {
			switch ( $request[ 'object' ] ) {
				case 'countries':
					return new \WP_REST_Response( apply_filters( 'esign/project/database/countries', [], false ) );
					break;
				case 'countries-name':
					return new \WP_REST_Response( array_values( (array) apply_filters( 'esign/project/database/countries', [], false ) ) );
					break;
				case 'countries-code':
					return new \WP_REST_Response( array_keys( (array) apply_filters( 'esign/project/database/countries', [], false ) ) );
					break;
				default:
