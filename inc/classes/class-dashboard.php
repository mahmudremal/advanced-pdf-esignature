...
class Dashboard {
    ...
    add_filter( 'esign/project/admin/allowedpage', [ $this, 'allowedpage' ], 10, 0 );
    add_filter( 'esign/project/admin/pagetree', [ $this, 'pageTree' ], 10, 1 );
    add_filter( 'esign/project/admin/title', [ $this, 'adminTitle' ], 10, 2 );
		add_action( 'admin_menu', [ $this, 'admin_menu' ], 10, 0 );
    add_action( 'wp_after_admin_bar_render', [ $this, 'wp_after_admin_bar_render' ], 10, 0 );
    add_action( 'esign/project/parts/call', [ $this, 'partsCall' ], 10, 1 );
    add_filter( 'esign/project/classes/rootnav', [ $this, 'rootnavClasses' ], 10, 1 );
    
    add_action( 'esign/project/parts/split', [ $this, 'partSplit' ], 10, 1 );
    add_action( 'esign/project/admin/notices', [ $this, 'adminNotices' ], 10, 1 );
    add_filter( 'esign/project/usermeta/defaults', [ $this, 'defaultUserMeta' ], 10, 1 );
    add_filter( 'esign/project/widgets/statustab', [ $this, 'statusTab' ], 10, 2 );
	}
  public function admin_menu() {
    add_menu_page(
      __(  'CRM Dashboard', 'esignbinding' ),     // page title
      __(  'Dashboard', 'esignbinding' ),     // menu title
      'manage_options',   // capability
      'crm_dashboard',     // menu slug  
      [ $this, 'crm_dashboard' ], // callback function
      ESIGNBINDING_ADDONS_BUILD_URI . '/icons/dashboard-alt.svg',
      10
    );
  }
  public function crm_dashboard() {
    ...
    <div class="wrap dashboard__body-start">
      <?php include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/crm_dashboard.php'; ?>
    </div>
    <?php
  }
  public function wp_after_admin_bar_render() {
    if( ! isset( $_GET[ 'page' ] ) || ! in_array( $_GET[ 'page' ], $this->allowedPages ) ) {return;}
    include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/bar_render.php';
  }
  ...
  public function pageTree( $args ) {
    return [
      ...
      'leads'     => [
        'title'   => __(  'Leads from Facebook', 'esignbinding' ),
      ...
      // 'payments' => [
      //   'title'   => __(  'Payments', 'esignbinding' ),
      ...
      //   'submenu' => [
      //     'stripe_logs' => [
      //       'title'   => __(  'Stripe logs', 'esignbinding' ),
      //       'submenu' => []
      //     ],
      //   ]
      // ],
      'archives' => [
        'title'   => __(  'Archives', 'esignbinding' ),
  ...
  public function partsCall( $position ) {
    ...
    switch( $position ) {
      case 'after_nav':
        include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/after_nav.php';
        break;
      case 'content':
        if( in_array( 'dashboard', [ $current_pathinfo[0], '' ] ) ) {
          include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/content.php';
        } else {
          do_action( 'esign/project/parts/split', $this->args );
        }
        break;
      case 'homecontent':
        if( in_array( 'dashboard', [ $current_pathinfo[0], '' ] ) ) {
          // include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/tasks.php';
          include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/leads.php';
        } else {
          do_action( 'esign/project/parts/split', $this->args );
        }
        break;
  ...
  public function partSplit( $args ) {
    if( $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] == 'edit' || $args[ 'split' ][1] == 'add' ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/editprofile.php';
    } else if( $args[ 'split' ][0] == '' || $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] === false ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/leads.php';
    } else if( $args[ 'split' ][0] == 'payments' && $args[ 'split' ][1] == false ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/stripe.php';
    } else if( $args[ 'split' ][1] == 'stripe_logs' ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/' . ( ( $args[ 'split' ][2] === false ) ? 'stripe_logs' : 'stripe_details' ) . '.php';
    } else if( $args[ 'split' ][0] == 'archives' ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/' . ( ( $args[ 'split' ][2] === false ) ? 'archives' : 'archive' ) . '.php';
    } else if( $args[ 'split' ][0] == 'notices' ) {
      include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/admin/notices.php';
    } else {}
  }
  public function adminTitle( $default, $args ) {
    ...
    if( $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] == 'edit' || $args[ 'split' ][1] == 'add' ) {
      return __( 'Edit Lead', 'esignbinding' );
    } else if( $args[ 'split' ][0] == '' || $args[ 'split' ][0] == 'leads' && $args[ 'split' ][1] === false ) {
      return __( 'Leads', 'esignbinding' );
    } else if( $args[ 'split' ][0] == 'payments' && $args[ 'split' ][1] == false ) {
      return __( 'Payments', 'esignbinding' );
    } else if( $args[ 'split' ][1] == 'stripe_logs' ) {
      return __( 'Stripe Logs', 'esignbinding' );
    } else if( $args[ 'split' ][0] == 'archives' ) {
      return __( 'Archive Uploaded', 'esignbinding' );
    } else if( $args[ 'split' ][0] == 'notices' ) {
      return __( 'Notices', 'esignbinding' );
    } else {return  $default;}
  }
  public function adminNotices() {
    $alert = (array) get_transient( 'esign/project/transiant/admin/' . get_current_user_id() );
    if( isset( $alert[ 'type' ] ) && isset( $alert[ 'message' ] ) ) {
      delete_transient( 'esign/project/transiant/admin/' . get_current_user_id() );
      ?>
      <!--begin::Alert-->
