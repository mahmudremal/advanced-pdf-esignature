...
class Shortcode {
	...
	public function checkout_video( $args ) {
		...
		if( ! $args[ 'post_id' ] || empty( $args[ 'post_id' ] ) ) {$errorHappens = __( 'Link that required video clip ID included.', 'esignbinding' );}
		$meta = get_post_meta( $args[ 'post_id' ], 'checkout_video_clip', true );
		if( ! isset( $meta[ 'full_path' ] ) || $meta[ 'full_path' ] === false || ! file_exists( $meta[ 'full_path' ] ) || is_dir( $meta[ 'full_path' ] ) ) {$errorHappens = __( 'Video file not found. Maybe expired :( If so, message is file removed permanently from server.', 'esignbinding' );}
		// if( ! is_user_logged_in() || get_current_user_id() != get_post_field( 'post_author', $args[ 'post_id' ] ) || ! current_user_can( 'manage_options' ) ) {$errorHappens = __( 'You don\'t have authorization to access this video. Only people who placed order or uploaded video and admin can access this video.', 'esignbinding' );}
