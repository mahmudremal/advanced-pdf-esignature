...
class Ftp {
    ...
    add_filter( 'esign/project/filesystem/mediadirectory', [ $this, 'getAllLocalFiles' ], 10, 2 );
		// add_action( 'init', [ $this, 'deleteFiles' ], 10, 0 );
	}
	public function initialize() {
    $this->lastError        = false;
    $this->ftpConnect       = false;
    $this->ftpServer        = apply_filters( 'esign/project/system/getoption', 'ftp-server', false );
    $this->ftpUsername      = apply_filters( 'esign/project/system/getoption', 'ftp-username', false );
    $this->ftpPassword      = apply_filters( 'esign/project/system/getoption', 'ftp-password', false );
    $this->ftpDirectory     = apply_filters( 'esign/project/system/getoption', 'ftp-remotedir', false );
    $this->localDirectory   = apply_filters( 'esign/project/system/getoption', 'ftp-localdir', false );
    if( $this->localDirectory === false || empty( $this->localDirectory ) ) {
      $this->localDirectory = apply_filters( 'esign/project/filesystem/uploaddir', false );
    }
	}
  public function syncFiles() {
    // if( ! apply_filters( 'esign/project/system/isactive', 'ftp-enable' ) ) {return;}
