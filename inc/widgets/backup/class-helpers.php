 ...
 */
 class Helpers {
    ...
    return isset( ESIGNBINDING_ADDONS_OPTIONS[ $option ] ) ? ESIGNBINDING_ADDONS_OPTIONS[ $option ] : $default;
	}
	/**
    ...
    return ( isset( ESIGNBINDING_ADDONS_OPTIONS[ $option ] ) && ESIGNBINDING_ADDONS_OPTIONS[ $option ] == 'on' );
	}
	/**
	...
	public function sendMail( $args = [] ) {
		...
		// $msg = [ 'status' => 'success', 'message' => __( get_FwpOption( 'msg_profile_edit_success_txt', 'Changes saved' ), ESIGNBINDING_ADDONS_TEXT_DOMAIN ) ];
	...
	public function uploadFile() {
		...
		if( isset( $_FILES[ 'blobFile' ] ) || isset( $_FILES[ 'file' ] ) ) {
			...
			// 	$error = sprintf( __( 'File is larger then allowed range. (%d)', 'esignbinding' ), $file[ 'size' ][0] );
			// }
			...
			// 	$error = sprintf( __( 'File format (%s) is not allowed.', 'esignbinding' ), $extension );
			// }
			...
			if( $error === false && move_uploaded_file( $file[ 'tmp_name' ][0], $file[ 'full_path' ] ) ) {
				...
				wp_send_json_success( [
					'message'			=> __( 'Uploaded successfully', 'esignbinding' ),
					'dropZone'		=> $meta
				], 200 );
			} else {
				$error = ( $error ) ? $error : __( 'Something went wrong while tring to upload short clip video.', 'esignbinding' );
	...
	public function removeFile() {
		...
		if( isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			wp_send_json_success( __( 'File removed from server.', 'esignbinding' ), 200 );
		} else {
			wp_send_json_error( __( 'Failed to delete. Maybe File not found on server or your request doesn\'t contain file data enough.', 'esignbinding' ), 200 );
		}
	}
	public function downloadFile() {
		...
		if( isset( $fileInfo[ 'full_url' ] ) && isset( $fileInfo[ 'full_path' ] ) && file_exists( $fileInfo[ 'full_path' ] ) && ! is_dir( $fileInfo[ 'full_path' ] ) ) {
			...
			print_r( $fileInfo );
			wp_die( __( 'File not found', 'esignbinding' ), __( '404 not found', 'esignbinding' ) );
		}
	}
	...
	public function dashboardPermalink( $id, $user = 'me' ) {
		if( ! defined( 'ESIGNBINDING_ADDONS_DASHBOARDPERMALINK' ) ) {
			...
			define( 'ESIGNBINDING_ADDONS_DASHBOARDPERMALINK', $dashboard_permalink );
		}
		$profile = ( apply_filters( 'esign/project/system/getoption', 'permalink-userby', 'id' ) == 'id' ) ? ESIGNBINDING_ADDONS_DASHBOARDPERMALINK . '/' . ( ( $id ) ? $id : 'me' ) : ESIGNBINDING_ADDONS_DASHBOARDPERMALINK . '/' . $user;
