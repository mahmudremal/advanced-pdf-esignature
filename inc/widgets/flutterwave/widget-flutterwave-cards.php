...
class GF_FlutterWave_Credit_Card_Field extends \GFPaymentAddOn {
	...
	public function pre_init() {
		...
		require_once untrailingslashit(ESIGNBINDING_ADDONS_DIR_PATH).'/inc/widgets/flutterwave/credit-card.php';
	}
	...
	public function scripts() {
		...
		$scripts = array(
			...
			array(
				...
				'strings'   => array(
					'no_active_frontend_feed'         => wp_strip_all_tags( __( 'The credit card field will initiate once the payment condition is met.', 'esignbinding' ) ),
					'requires_action'                 => wp_strip_all_tags( __( 'Please follow the instructions on the screen to validate your card.', 'esignbinding' ) ),
					'create_payment_intent_nonce'     => wp_create_nonce( 'gf_stripe_create_payment_intent' ),
					'ajaxurl'                         => admin_url( 'admin-ajax.php' ),
					'payment_incomplete'              => wp_strip_all_tags( __( 'Please enter all required payment information.', 'esignbinding' ) ),
					'failed_to_create_draft'          => wp_strip_all_tags( __( 'We could not process your request at the moment.', 'esignbinding' ) ),
					'failed_to_create_initial_intent' => wp_strip_all_tags( __( 'Payment information field failed to be displayed, please contact support.', 'esignbinding' ) ),
					'failed_to_confirm_intent'        => wp_strip_all_tags( __( 'The payment gateway failed to process the request. Please use a different payment method.', 'esignbinding' ) ),
					'rate_limit_exceeded'             => esc_html__( 'We are not able to process your payment request at the moment. Please try again later.', 'esignbinding' ),
					'payment_element_intent_failure'  => esc_html__( 'The payment has failed', 'esignbinding' ),
			...
			array(
				...
				'strings'   => array(
					...
					'validation_error'                => wp_strip_all_tags( __( 'Error validating this key. Please try again later.', 'esignbinding' ) ),
					'switch_account_disabled_message' => wp_strip_all_tags( __( 'In order to switch accounts, you must first save this feed by clicking the "Update Settings" button below', 'esignbinding' ) ),
					'disconnect'                      => array(
						'site'    => wp_strip_all_tags( __( 'Are you sure you want to disconnect from Stripe for this website?', 'esignbinding' ) ),
						'feed'    => wp_strip_all_tags( __( 'Are you sure you want to disconnect from Stripe for this feed?', 'esignbinding' ) ),
						'account' => wp_strip_all_tags( __( 'Are you sure you want to disconnect all Gravity Forms sites connected to this Stripe account?', 'esignbinding' ) ),
					),
					...
					'refund'                          => wp_strip_all_tags( __( 'Are you sure you want to refund this payment?', 'esignbinding' ) ),
					'refund_nonce'                    => wp_create_nonce( 'gf_stripe_refund' ),
					'refund_processing'               => wp_strip_all_tags( __( 'Processing refund', 'esignbinding' ) ),
					'refund_complete'                 => wp_strip_all_tags( __( 'Transaction successfully refunded', 'esignbinding' ) ),
					'capture_confirm'                 => wp_strip_all_tags( __( 'Are you sure you want to capture this payment?', 'gravityformstripe' ) ),
					'capture_processing'              => wp_strip_all_tags( __( 'Processing capture', 'esignbinding' ) ),
					'capture_complete'                => wp_strip_all_tags( __( 'Transaction successfully captured', 'esignbinding' ) ),
				),
			),
			array(
				...
				'strings'   => array(
					'payment_element_error'            => wp_strip_all_tags( __( 'Please check your Stripe settings', 'esignbinding' ) ),
					'payment_element_currency'         => strtolower( GFCommon::get_currency() ),
					'api_key'                          => $this->get_publishable_api_key(),
					'field_position_validation_error'  => wp_strip_all_tags( __( 'When the additional payment methods option is enabled, the Stripe field must be on the last page of the form.', 'esignbinding' ) ),
					'stripe_connect_enabled'           => $this->is_stripe_connect_enabled() === true,
					'payment_element_supported'        => $this->is_payment_element_supported(),
					'payment_element_disabled_message' => wp_strip_all_tags( __( 'To enable additional payment methods, you must update Gravity Forms to the latest version.', 'esignbinding' ) ),
				),
			),
		);
	...
	public function add_payment_method_setting( $placement, $form_id ) {
		...
		<li class="enable_multiple_payment_methods_setting field_setting">
			<label for="rules" class="section_label">
				<?php esc_html_e( 'Payment Methods', 'esignbinding' ); ?>
			...
			<div>
				...
				<label for="field_enable_multiple_payment_methods" class="inline">
					<?php esc_html_e( 'Enable additional payment methods', 'esignbinding' ); ?>
				</label>
				<div id="field_multiple_payment_methods_description">
					<?php
					// translators: variables are the markup to generate a link.
					printf( esc_html__( 'Available payment methods can be configured in your %1$sStripe Dashboard%2$s.', 'esignbinding' ), '<a href="https://dashboard.stripe.com/settings/payment_methods" target="_blank">', '</a>' );
					?>
				</div>
			</div>
			<br>
			<div id="link_email_field_container">
				...
				<div>
					<?php
					// translators: variables are the markup to generate a link.
					printf( esc_html__( 'Link is a payment method that enables your customers to save their payment information so they can use it again on any site that uses Stripe Link. %1$sLearn more about Link%2$s.', 'esignbinding' ), '<a href="https://stripe.com/docs/payments/link" target="_blank">', '</a>' );
					?>
				</div>
			</div>
			<?php } else { ?>
				<div>
					<?php
					// translators: variables are the markup to generate a link.
					printf( esc_html__( 'This option is disabled because Stripe is authenticated using API keys instead of Stripe Connect. To take advantage of additional payment methods, re-authenticate using Stripe Connect. %1$sLearn more about Stripe payment element%2$s.', 'esignbinding' ), '<a href="https://stripe.com/docs/payments/payment-element" target="_blank">', '</a>' );
					?>
				</div>
			<?php } ?>
		</li>
	...
	public function get_billing_portal_handler() {
		...
		if ( $this->billing_portal_handler instanceof GF_Flutterwave_Billing_Portal === false ) {
			require_once untrailingslashit(ESIGNBINDING_ADDONS_DIR_PATH).'/inc/widgets/flutterwave/billing-portal.php';
	...
	public function ajax_validate_secret_key() {
		...
		if ( ! GFCommon::current_user_can_any( $this->_capabilities_settings_page ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Access denied.', 'esignbinding' ) ) );
		}
	...
	public function plugin_settings_fields() {
		$fields = array(
			array(
				'title'  => esc_html__( 'Stripe Account', 'esignbinding' ),
		...
		if ( version_compare( \GFFormsModel::get_database_version(), '2.4-beta-1', '>=' ) ) {
			$fields[] = array(
				'title'       => esc_html__( 'Payment Collection', 'esignbinding' ),
				'description' => $this->get_checkout_method_section_description(),
				'fields'      => array(

					array(
						'name'          => 'checkout_method',
						'label'         => esc_html__( 'Payment Collection Method', 'esignbinding' ),
						'type'          => 'radio',
						'default_value' => 'stripe_elements',
						'choices'       => array(
							array(
								'label'   => esc_html__( 'Stripe Field (Elements, SCA-ready)', 'esignbinding' ),
								'value'   => 'stripe_elements',
								'tooltip' => '<h6>' . esc_html__( 'Stripe Field', 'esignbinding' ) . '</h6>' .
								             '<p>' . esc_html__( 'Select this option to use a Stripe field hosted by Stripe. The Stripe field can be embedded in a form and supports credit card payments as well as other payment methods such as Google Pay, Apple Pay and bank transfers.', 'esignbinding' ) .
								             '</p><p>' .
								             /* translators: 1. Open link tag 2. Close link tag */
								             sprintf( esc_html__( 'Stripe Field is ready for %1$sStrong Customer Authentication%2$s for European customers.', 'esignbinding' ), '<a href="https://stripe.com/docs/strong-customer-authentication" target="_blank">', '</a>' ) .
								             '</p>',
							),
							array(
								'label'   => esc_html__( 'Stripe Payment Form (Stripe Checkout, SCA-ready)', 'esignbinding' ),
								'value'   => 'stripe_checkout',
								'tooltip' => '<h6>' . esc_html__( 'Stripe Payment Form', 'esignbinding' ) . '</h6>' .
								             '<p>' . esc_html__( 'Select this option to collect all payment information in a separate page hosted by Stripe. This option is the simplest to implement since it doesn\'t require a Stripe field in your form. Stripe Checkout supports credit card payments as well as other payment methods such as Google Pay, Apple Pay and bank transfers.', 'esignbinding' ) .
								             '</p><p>' .
								             /* translators: 1. Open link tag 2. Close link tag */
								             sprintf( esc_html__( 'Stripe Checkout is ready for %1$sStrong Customer Authentication%2$s for European customers.', 'esignbinding' ), '<a href="https://stripe.com/docs/strong-customer-authentication" target="_blank">', '</a>' ) .
								             '</p>',
							),
						),
					),
	...
	public function api_settings_fields() {
		...
		$fields = array(
			array(
				'name'       => 'connected_to',
				'label'      => esc_html__( 'Connected to Stripe as', 'esignbinding' ),
				'type'       => 'connected_to',
				'dependency' => array( $this, 'is_detail_page' ),
				'tooltip'    => '<h6>' . esc_html__( 'Connected to Stripe as', 'esignbinding' ) . '</h6>' . esc_html__( 'The Stripe account this feed is currently connected to.', 'esignbinding' ),
			),
			array(
				'name'          => 'api_mode',
				'label'         => esc_html__( 'Mode', 'esignbinding' ),
				'type'          => 'radio',
				'default_value' => $this->get_api_mode( $this->get_plugin_settings() ),
				'choices'       => array(
					array(
						'label' => esc_html__( 'Live', 'esignbinding' ),
						'value' => 'live',
					),
					array(
						'label' => esc_html__( 'Test', 'esignbinding' ),
						'value' => 'test',
					),
				),
				'horizontal'    => true,
		...
		if ( ! $this->is_detail_page() && ! $this->is_stripe_connect_enabled() ) {
			$legacy_connect_fields = array(
				array(
					...
					'label'    => esc_html__( 'Test Publishable Key', 'esignbinding' ),
					'type'     => 'text',
					'class'    => 'medium',
				...
				array(
					...
					'label'      => esc_html__( 'Test Secret Key', 'esignbinding' ),
					'type'       => 'text',
					'input_type' => 'password',
					'class'      => 'medium',
				...
				array(
					...
					'label'    => esc_html__( 'Live Publishable Key', 'esignbinding' ),
					'type'     => 'text',
					'class'    => 'medium',
				...
				array(
					...
					'label'      => esc_html__( 'Live Secret Key', 'esignbinding' ),
					'type'       => 'text',
					'input_type' => 'password',
					'class'      => 'medium',
		...
		$webhook_fields = array(
			array(
				...
				'label'       => esc_html__( 'Webhooks Enabled?', 'esignbinding' ),
				'type'        => 'checkbox',
				'horizontal'  => true,
				...
				'choices'     => array(
					array(
						'label' => esc_html__( 'I have enabled the Gravity Forms webhook URL in my Stripe account.', 'esignbinding' ),
						'value' => 1,
						'name'  => 'webhooks_enabled',
					),
				),
			...
			array(
				...
				'label'               => esc_html__( 'Test Signing Secret', 'esignbinding' ),
				'type'                => 'text',
				'input_type'          => 'password',
				'class'               => 'medium',
				'dependency'          => $this->get_webhooks_dependency(),
			...
			array(
				...
				'label'               => esc_html__( 'Live Signing Secret', 'esignbinding' ),
				'type'                => 'text',
				'input_type'          => 'password',
				'class'               => 'medium',
				'dependency'          => $this->get_webhooks_dependency(),
	...
	public function settings_ssl_error( $field, $echo = true ) {
		...
		$html .= '<h4>' . esc_html__( 'SSL Certificate Required', 'esignbinding' ) . '</h4>';
		/* Translators: 1: Open link tag 2: Close link tag */
		$html .= sprintf( esc_html__( 'Make sure you have an SSL certificate installed and enabled, then %1$sclick here to reload the settings page%2$s.', 'esignbinding' ), '<a href="' . $this->get_settings_page_url() . '">', '</a>' );
		$html .= '</div>';
	...
	public function get_webhooks_section_description() {
		...
		<a href="javascript:void(0);"
				onclick="tb_show('Webhook Instructions', '#TB_inline?width=500&inlineId=stripe-webhooks-instructions', '');" onkeypress="tb_show('Webhook Instructions', '#TB_inline?width=500&inlineId=stripe-webhooks-instructions', '');"><?php esc_html_e( 'View Instructions', 'esignbinding' ); ?></a></p>

		<div id="stripe-webhooks-instructions" style="display:none;">
			<ol class="stripe-webhooks-instructions">
				<li>
					<?php esc_html_e( 'Click the following link and log in to access your Stripe Webhooks management page:', 'esignbinding' ); ?>
					<br/>
					<a href="https://dashboard.stripe.com/account/webhooks" target="_blank">https://dashboard.stripe.com/account/webhooks</a>
				</li>
				<li><?php esc_html_e( 'Click the "Add Endpoint" button above the list of Webhook URLs.', 'esignbinding' ); ?></li>
				<li>
					<?php esc_html_e( 'Enter the following URL in the "URL to be called" field:', 'esignbinding' ); ?>
					<code><?php echo $this->get_webhook_url( $this->get_current_feed_id() ); ?></code>
				</li>
				<li><?php esc_html_e( 'If offered the choice, select the latest API version.', 'esignbinding' ); ?></li>
				<li><?php esc_html_e( 'Click the "receive all events" link.', 'esignbinding' ); ?></li>
				<li><?php esc_html_e( 'Click the "Add Endpoint" button to save the webhook.', 'esignbinding' ); ?></li>
				<li><?php esc_html_e( 'Copy the signing secret of the newly created webhook on Stripe and paste to the setting field.', 'esignbinding' ); ?></li>
			</ol>
	...
	public function get_checkout_method_section_description() {
		ob_start();
		?>
		<p><?php esc_html_e( 'Select how payment information will be collected. You can select one of the Stripe hosted solutions (Stripe Credit Card or Stripe Checkout) which simplifies the PCI compliance process with Stripe.', 'esignbinding' ); ?></p>

		<?php
		if ( $this->get_form_with_deprecated_cc_field() ) {
			/* translators: Placeholders represent opening and closing link tags. */
			echo '<p>' . sprintf( esc_html__( 'The Gravity Forms Credit Card Field was deprecated in the Stripe Add-On in version 3.4. Forms that are currently using this field will stop working in a future version. Refer to %1$sthis guide%2$s for more information about this change.', 'esignbinding' ), '<a href="https://docs.gravityforms.com/deprecation-of-the-gravity-forms-credit-card-field/" target="_blank">', '</a>' ) . '</p>';
		}
	...
	public function settings_auth_token_button( $field, $echo = true ) {
		...
		$connect_button = sprintf(
			...
			esc_html__( 'Click here to authenticate with Stripe', 'esignbinding' ),
		...
		if ( $this->requires_reauthentication() ) {
			...
			$deprecated_auth_message .= '<p>' . esc_html__( 'You are currently logged in to Stripe using a deprecated authentication method.', 'esignbinding' ) . '</p>';
			/* Translators: 1: Open strong tag 2: Close strong tag */
			$deprecated_auth_message .= '<p>' . sprintf( esc_html__( '%1$sPlease login to your Stripe account via Stripe Connect using the button below.%2$s It is a more secure authentication method and will be required for upcoming features of the Stripe Add-on.', 'esignbinding' ), '<strong>', '</strong>' ) . '</p>';
			$deprecated_auth_message .= '</div>';
		}

		// translators: Placeholders represent wrapping link tag.
		$learn_more_message = '<p>' . sprintf( esc_html__( '%1$sLearn more%2$s about connecting with Stripe.', 'esignbinding' ), '<a target="_blank" href=" https://docs.gravityforms.com/faq-authenticating-with-stripe/">', '</a>' ) . '</p>';
		...
		if ( $is_valid ) {
			if ( ! rgblank( $this->get_stripe_user_id( $settings, $api_mode ) ) && ! $this->is_detail_page() ) {
				...
				if ( ! $this->is_detail_page() && ! empty( $display_name ) ) {
					$html .= '<p class="connected_to_stripe_text">' . esc_html__( 'Connected to Stripe as', 'esignbinding' ) . ' <strong>' . $display_name . '</strong>.</p>';
				}
				$html .= sprintf(
					' <a href="#" class="button gform_stripe_deauth_button" data-fid="%2$s" data-id="%3$s" data-mode="%4$s">%1$s</a>',
					esc_html__( 'Disconnect your Stripe account', 'esignbinding' ),
				...
				$html .= sprintf( esc_html__( 'Disconnect this %s only', 'esignbinding' ), $target );
				$html .= '</label></p>';
				$html .= '<p><label for="' . $api_mode . '_deauth_scope1"><input type="radio" name="deauth_scope" value="account" id="' . $api_mode . '_deauth_scope1">' . esc_html__( 'Disconnect all Gravity Forms sites connected to this Stripe account', 'esignbinding' ) . '</label></p>';
				$html .= sprintf(
	...
	public function settings_connected_to() {
		...
		if ( $is_valid ) {
			...
			if ( ! empty( $display_name ) ) {
				$html .= sprintf(
					esc_html__( '%1$s%2$s%3$s', 'esignbinding' ),
					'<strong>',
					$display_name,
					'</strong>'
				);
		...
		if ( ! $this->requires_reauthentication() ) {
			...
			$html     .= esc_html__( 'Switch Accounts', 'esignbinding' );
			$html     .= '</a>';
		}
	...
	public function settings_stripe_checkout_logo() {
		/* Translators: 1. Open link tag. 2. Close link tag. */
		$html = sprintf( esc_html__( 'Logo can be configured on %1$sStripe\'s branding page%2$s.', 'esignbinding' ), '<a href="https://dashboard.stripe.com/account/branding" target="_blank">', '</a>' );
	...
	public function maybe_update_auth_tokens() {
		...
		if ( rgpost( 'state' ) && ! wp_verify_nonce( rgar( $payload, 'state' ), $this->get_authentication_state_action() ) ) {
			// Add error message.
			GFCommon::add_error_message( esc_html__( 'Unable to connect to Stripe due to mismatched state.', 'esignbinding' ) );
		...
		if ( rgpost( 'auth_error' ) || isset( $payload['auth_error'] ) ) {
			// Add error message.
			GFCommon::add_error_message( esc_html__( 'Unable to authenticate with Stripe.', 'esignbinding' ) );
		}
	}
	...
	public function maybe_display_capture_button( $form_id, $entry ) {
		...
		printf(
			...
			esc_html__( 'Capture Payment', 'esignbinding' ),
			esc_html__( 'Capturing payment', 'esignbinding' )
		);
	}
	...
	public function maybe_display_update_authentication_message( $return = false, $classes = 'notice notice-error' ) {
		...
		if ( $this->requires_reauthentication() ) {
			$message = sprintf(
			/* Translators: 1: Open link tag 2: Close link tag */
				esc_html__( 'You are currently logged in to Stripe using a deprecated authentication method. %1$sRe-authenticate your Stripe account%2$s.', 'esignbinding' ),
	...
	private function display_deprecated_cc_field_warning( $form ) {
		...
		/* translators: 1: Open strong tag, 2: Close strong tag, 3: Form title, 4: Open link tag, 5: Close link tag. */
			esc_html__( '%1$sImportant%2$s: The form %3$s is using a deprecated payment collection method for Stripe that will stop working in a future version. Take action now to continue collecting payments. %4$sLearn more.%5$s', 'esignbinding' ),
			'<strong>',
			'</strong>',
	...
	public function requires_stripe_card_message() {
		...
		return sprintf( esc_html__( "You must add a Stripe Card field to your form before creating a feed. Let's go %sadd one%s!", 'esignbinding' ), "<a href='" . esc_url( $url ) . "'>", '</a>' );
	}
	...
	public function feed_settings_fields() {
		...
		$customer_info_field = array(
			...
			'label'      => esc_html__( 'Customer Information', 'esignbinding' ),
			'type'       => 'field_map',
			'dependency' => array(
			...
			'field_map'  => array(
				array(
					'name'       => 'email',
					'label'      => esc_html__( 'Email', 'esignbinding' ),
					'required'   => true,
					'field_type' => array( 'email', 'hidden' ),
					'tooltip'    => '<h6>' . esc_html__( 'Email', 'esignbinding' ) . '</h6>' . esc_html__( 'You can specify an email field and it will be sent to Stripe as the customer\'s email.', 'esignbinding' ),
				),
				array(
					'name'     => 'description',
					'label'    => esc_html__( 'Description', 'esignbinding' ),
					'required' => false,
				),
				array(
					'name'       => 'coupon',
					'label'      => esc_html__( 'Coupon', 'esignbinding' ),
					'required'   => false,
					'field_type' => array( 'coupon', 'text' ),
					'tooltip'    => '<h6>' . esc_html__( 'Coupon', 'esignbinding' ) . '</h6><p>' . esc_html__( 'Select which field contains the coupon code to be applied to the recurring charge(s). The coupon must also exist in your Stripe Dashboard.', 'esignbinding' ) . '</p><p>' . esc_html__( 'If you use Stripe Checkout, the coupon won\'t be applied to your first invoice.', 'esignbinding' ) . '</p>',
				),
			),
		);
		...
		if ( 'subscription' === $this->get_setting( 'transactionType' ) ) {
			$info = esc_html__( 'You will see this data when viewing a customer page.', 'esignbinding' );
		} else {
			$info = esc_html__( 'You will see this data when viewing a payment page.', 'esignbinding' );
		}
		...
		$custom_meta = array(
			array(
				'name'                => 'metaData',
				'label'               => esc_html__( 'Metadata', 'esignbinding' ),
				'type'                => 'dynamic_field_map',
				'limit'               => 50,
				'exclude_field_types' => array( 'creditcard', 'stripe_creditcard' ),
				'tooltip'             => '<h6>' . esc_html__( 'Metadata', 'esignbinding' ) . '</h6>' . esc_html__( 'You may send custom meta information to Stripe. A maximum of 50 custom keys may be sent. The key name must be 40 characters or less, and the mapped data will be truncated to 500 characters per requirements by Stripe. ' . $info, 'esignbinding' ),
		...
		$trial_field = array(
			'name'	 => 'trial',
			'label'  => esc_html__( 'Trial', 'esignbinding' ),
			'type'   => 'trial',
			'hidden' => false,
			'tooltip' => '<h6>' . esc_html__( 'Trial Period', 'gravityforms' ) . '</h6>' . esc_html__( 'Enable a trial period.  The user\'s recurring payment will not begin until after this trial period.', 'esignbinding' )
		);
		...
		$trial_period_field = array(
			'name'                => 'trialPeriod',
			'label'               => esc_html__( 'Trial Period', 'esignbinding' ),
			'type'                => 'trial_period',
			'validation_callback' => array( $this, 'validate_trial_period' ),
		);

		if ( $this->_has_settings_renderer ) {
			$trial_period_field[ 'append' ] = esc_html__( 'days', 'esignbinding' );
		} else {
			$trial_period_field[ 'style' ]       = 'width:40px;text-align:center;';
			$trial_period_field[ 'after_input' ] = '&nbsp;' . esc_html__( 'days', 'esignbinding' );
		}
		...
		$subscription_name_field = array(
			...
			'label'   => esc_html__( 'Subscription Name', 'esignbinding' ),
			'type'    => 'text',
			'class'   => 'medium merge-tag-support mt-hide_all_fields mt-position-right mt-exclude-creditcard-stripe_creditcard',
			'tooltip' => '<h6>' . esc_html__( 'Subscription Name', 'esignbinding' ) . '</h6>' . esc_html__( 'Enter a name for the subscription. It will be displayed on the payment form as well as the Stripe dashboard.', 'esignbinding' ),
		);
		...
		if ( $this->has_stripe_card_field() ) {
			$default_settings[ $section_index ] = array(
				'title'      => esc_html__( 'Stripe Credit Card Field Settings', 'esignbinding' ),
				'dependency' => array(
					'field'  => 'transactionType',
				...
				'fields'     => array(
					array(
						...
						'label'     => esc_html__( 'Billing Information', 'esignbinding' ),
						'tooltip'   => '<h6>' . esc_html__( 'Billing Information', 'esignbinding' ) . '</h6>' . esc_html__( 'Map your Form Fields to the available listed fields. The address information will be sent to Stripe.', 'esignbinding' ),
						'type'      => 'field_map',
			...
			$default_settings[ $section_index ] = array(
				'title'       => esc_html__( 'Stripe Payment Form Settings', 'esignbinding' ),
				'description' => esc_html__( 'The following settings control information displayed on the Stripe hosted payment page that is displayed after the form is submitted.', 'esignbinding' ),
				'dependency'  => array(
					'field'  => 'transactionType',
					'values' => array( 'subscription', 'product', 'donation' ),
				),
		...
		if ( 'product' === $this->get_setting( 'transactionType' ) ) {
			...
			$receipt_settings = array(
				'name'    => 'receipt',
				'label'   => esc_html__( 'Stripe Receipt', 'esignbinding' ),
				'type'    => 'receipt',
				'tooltip' => '<h6>' . esc_html__( 'Stripe Receipt', 'esignbinding' ) . '</h6>' . esc_html__( 'Stripe can send a receipt via email upon payment. Select an email field to enable this feature.', 'esignbinding' ),
			);
	...
	public function get_stripe_payment_form_settings() {
		$settings = array(
			array(
				'name'  => 'logo',
				'label' => esc_html__( 'Logo', 'esignbinding' ),
				'type'  => 'stripe_checkout_logo',
			),
			array(
				'name'       => 'customer_email',
				'label'      => esc_html__( 'Customer Email', 'esignbinding' ),
				'type'       => 'receipt',
				'dependency' => array(
					'field'  => 'transactionType',
					'values' => array( 'product' ),
				),
				'tooltip'    => '<h6>' . esc_html__( 'Customer Email', 'esignbinding' ) . '</h6>' . esc_html__( 'You can specify an email field and it will be sent to Stripe as the customer\'s email.', 'esignbinding' ),
			),
			array(
				'name'          => 'billingAddress',
				'label'         => esc_html__( 'Billing Address', 'esignbinding' ),
				'type'          => 'radio',
				'tooltip'       => '<h6>' . esc_html__( 'Billing Address', 'esignbinding' ) . '</h6>' . esc_html__( 'When enabled, Stripe Checkout will collect the customer\'s billing address for you.', 'esignbinding' ),
				'horizontal'    => true,
				'choices'       => array(
					array(
						'label' => esc_html__( 'Enabled', 'esignbinding' ),
						'value' => 1,
					),
					array(
						'label' => esc_html__( 'Disabled', 'esignbinding' ),
						'value' => 0,
					),
				),
				'default_value' => 0,
	...
	public function settings_receipt( $field, $echo = true ) {
		...
		if ( $field['name'] === 'receipt' ) {
			...
			$first_choice = array(
				'label' => esc_html__( 'Do not send receipt', 'esignbinding' ),
				'value' => '',
			);
		} elseif ( $field['name'] === 'customer_email' ) {
			// Prepare first field choice and get form fields as choices.
			$first_choice = array(
				'label' => esc_html__( 'Do not set customer email', 'esignbinding' ),
				'value' => '',
			);
	...
	public function validate_trial_period( $field ) {
		...
		if ( $settings['trial_enabled'] && ( empty( $settings['trialPeriod'] ) || ! ctype_digit( $settings['trialPeriod'] ) ) ) {
			$this->set_field_error( $field, esc_html__( 'Please enter a valid number of days.', 'esignbinding' ) );
		}
	...
	public function validate_custom_meta( $field ) {
		...
		if ( $meta_count > 50 ) {
			$meta_count_error = array( esc_html__( 'You may only have 50 custom keys.' ), 'esignbinding' );
		...
		foreach ( $meta_data as $meta ) {
			if ( empty( $meta['custom_key'] ) && ! empty( $meta['value'] ) ) {
				$this->set_field_error( $custom_meta_field, esc_html__( "A field has been mapped to a custom key without a name. Please enter a name for the custom key, remove the metadata item, or return the corresponding drop down to 'Select a Field'.", 'esignbinding' ) );
				break;
			} else if ( strlen( $meta['custom_key'] ) > 40 ) {
				$this->set_field_error( $custom_meta_field, sprintf( esc_html__( 'The name of custom key %s is too long. Please shorten this to 40 characters or less.', 'esignbinding' ), $meta['custom_key'] ) );
				break;
			}
		}
	...
	public function validate_webhook_signing_secret( $field, $field_setting ) {
		...
		if ( ! empty( $field_setting ) && strpos( $field_setting, 'whsec_' ) === false ) {
			$this->set_field_error( $field, esc_html__( 'Please use the correct webhook signing secret, which should start with "whsec_".', 'esignbinding' ) );
		}
	}
	...
	public function supported_billing_intervals() {
		...
		return array(
			'day'   => array( 'label' => esc_html__( 'day(s)', 'esignbinding' ), 'min' => 1, 'max' => 365 ),
			'week'  => array( 'label' => esc_html__( 'week(s)', 'esignbinding' ), 'min' => 1, 'max' => 12 ),
			'month' => array( 'label' => esc_html__( 'month(s)', 'esignbinding' ), 'min' => 1, 'max' => 12 ),
			'year'  => array( 'label' => esc_html__( 'year(s)', 'esignbinding' ), 'min' => 1, 'max' => 1 ),
		);
	...
	public function supported_notification_events( $form ) {
		...
		return array(
			'complete_payment'          => esc_html__( 'Payment Completed', 'esignbinding' ),
			'refund_payment'            => esc_html__( 'Payment Refunded', 'esignbinding' ),
			'fail_payment'              => esc_html__( 'Payment Failed', 'esignbinding' ),
			'create_subscription'       => esc_html__( 'Subscription Created', 'esignbinding' ),
			'cancel_subscription'       => esc_html__( 'Subscription Canceled', 'esignbinding' ),
			'add_subscription_payment'  => esc_html__( 'Subscription Payment Added', 'esignbinding' ),
			'fail_subscription_payment' => esc_html__( 'Subscription Payment Failed', 'esignbinding' ),
		);
	...
	public function pre_validation( $result, $value, $form, $field ) {
		...
		if ( $field->type == 'creditcard' && rgpost( 'stripe_credit_card_last_four' ) ) {
			...
			if ( ! $card_type || $card_type === 'false' ) {
				$card_type = __( 'Unknown', 'esignbinding' );
			}
			...
			if ( $field->type == 'creditcard' && ! $field->is_card_supported( $card_slug ) ) {
				...
				$result['message']  = sprintf( esc_html__( 'Card type (%s) is not supported. Please enter one of the supported credit cards.', 'esignbinding' ), $card_type );
			} else {
	...
	public function card_type_validation_message( $validation_message, $form ) {
		...
		$message = sprintf( esc_html__( 'Card type (%1$s) is not supported. Please enter one of the supported credit cards.', 'esignbinding' ), $card_type );
	...
	public function stripe_checkout_error_message( $validation_message = '', $form = array() ) {
		return $this->get_validation_error_markup(
			esc_html__( 'There was a problem with your submission.', 'esignbinding' ) . ' ' . $this->get_authorization_error_message(),
			$form
		);
	}
	...
	public function stripe_general_validation_error_message( $validation_message = '', $form = array() ) {
		...
		if ( ! $message ) {
			return esc_html__( 'There was a problem with your submission. Please try again later.', 'esignbinding' );
		}

		return $this->get_validation_error_markup(
			esc_html__( 'There was a problem with your submission:', 'esignbinding' ) . ' ' . $message,
			$form
		);
	}
		...
		if (
			...
			if ( ! is_wp_error( $result ) ) {
				...
				if ( $result->amount !== $expected_amount ) {
					...
					return $this->authorization_error( esc_html__( 'Your payment attempt has failed. Please enter your card details and try again.', 'esignbinding' ) );
				}
			...
			if ( ! $payment_element_enabled && $result->status === 'requires_action' ) {
				...
				$error = $this->authorization_error( esc_html__( '3D Secure authentication is required for this payment. Please follow the instructions on the page to continue.', 'esignbinding' ) );
				...
				return $this->authorization_error( esc_html__( 'The payment has been canceled', 'esignbinding' ) );
			} else {
	...
	public function create_checkout_session( $feed, $submission_data, $form, $entry ) {
		...
		if ( ! $is_subscription ) {
			...
			if ( $discount_amt > 0 ) {
				 ...
				 * @param array  $entry           The entry object currently being processed.
				 */
				$line_items_name = apply_filters( 'gform_stripe_discounted_line_items_name', esc_html__( 'Payment with Discounts', 'esignbinding' ), $feed, $submission_data, $form, $entry );
				...
				$line_items                 = array(
					array(
						...
						'price_data' => array(
							...
							'currency'     => $currency,
							'product_data' => array(
								'name' => esc_html__( 'Setup Fee', 'esignbinding' ),
							),
						),
					),
				);
		...
		return $this->authorization_error( esc_html__( 'Unable to create Stripe Checkout session.', 'esignbinding' ) );
	}
	...
	public function capture( $auth, $feed, $submission_data, $form, $entry ) {
		...
		if ( $payment_element_enabled || $this->is_payment_intent( $response->id ) ) {
			...
			if ( is_wp_error( $intent ) ) {
				...
				return array(
					'is_success'    => false,
					'error_message' => esc_html__( 'Cannot get payment intent data.', 'esignbinding' ),
				);
			}
			...
			if ( is_wp_error( $intent ) ) {
				...
				return array(
					'is_success'    => false,
					'error_message' => esc_html__( 'Cannot update payment intent data.', 'esignbinding' ),
				);
			}
			...
			if ( $payment_element_enabled === false ) {
				...
				if ( is_wp_error( $intent ) ) {
					...
					return array(
						'is_success'    => false,
						'error_message' => esc_html__( 'Cannot capture payment intent data.', 'esignbinding' ),
					);
				}
			}

			if ( $intent->status !== 'succeeded' ) {
				$payment = $this->authorization_error( esc_html__( 'Cannot capture the payment; the payment intent status is ', 'esignbinding' ) . $intent->status );
			} else {
				$payment = array(
					'is_success'     => true,
			...
			if ( is_wp_error( $charge ) ) {
				...
				return array(
					'is_success'    => false,
					'error_message' => esc_html__( 'Unable to capture the charge.', 'esignbinding' ),
				);
			}
			...
			if ( is_wp_error( $charge ) ) {
				...
				return array(
					'is_success'    => false,
					'error_message' => esc_html__( 'Unable to save the charge.', 'esignbinding' ),
				);
			}
	...
	public function process_subscription( $authorization, $feed, $submission_data, $form, $entry ) {
		...
		if ( $customer_id ) {
			...
			$authorization['subscription']['error_message'] = esc_html__( 'Failed to get the customer ID from Stripe.', 'esignbinding' );
		}
	...
	public function complete_checkout_session( $session, $entry, $feed, $form ) {
		...
		if ( $subscription_id = rgar( $session, 'subscription' ) ) {
			...
			if ( in_array( rgar( $subscription, 'status' ), array( 'active', 'trialing' ), true ) && $payment_status !== 'Active' ) {
				...
				$action['note']   .= sprintf( __( 'Subscription payment has been paid. Amount: %s. Subscription Id: %s', 'esignbinding' ), $amount_formatted, $action['subscription_id'] );
	...
	public function get_stripe_js_error() {
		...
		if ( isset( $response->error ) ) {
			...
			return esc_html__( 'Unable to authorize card. No response from Stripe.js.', 'esignbinding' );
		}
	...
	public function callback() {
		...
		switch ( $type ) {
			...
			case 'invoice.payment_succeeded':
				...
				if ( ! $subscription ) {
					return new WP_Error( 'invalid_request', sprintf( __( 'Subscription line item not found in request', 'esignbinding' ) ) );
				}
			...
			case 'checkout.session.async_payment_succeeded':
				...
				if ( is_wp_error( $session ) ) {
					...
					return new WP_Error( 'invalid_stripe_session', esc_html__( 'Invalid Checkout Session', 'esignbinding' ) . ': ' . $session->get_error_message() );
				}
	...
	public function get_wrong_feed_wp_error( $entry_id ) {
		return new WP_Error(
			...
			sprintf( __( 'Entry %d was not processed by feed %d. Webhook cannot be processed.', 'esignbinding' ), $entry_id, $_GET['fid'] ),
	...
	public function get_entry_not_found_wp_error( $type, $action, $event ) {
		$message     = sprintf( __( 'Entry for %s id: %s was not found. Webhook cannot be processed.', 'esignbinding' ), $type, rgar( $action, $type . '_id' ) );
		$status_code = 200;
	...
	public function get_webhook_event() {
		...
		if ( $error_message ) {
			...
			$message = __( 'Invalid request. Webhook could not be processed.', 'esignbinding' ) . ' ' . $error_message;

			return new WP_Error( 'invalid_request', $message, array( 'status_header' => 400 ) );
		}

		if ( $is_test_event ) {
			return new WP_Error( 'test_webhook_succeeded', __( 'Test webhook succeeded. Your Stripe Account and Stripe Add-On are configured correctly to process webhooks.', 'esignbinding' ), array( 'status_header' => 200 ) );
		}
	...
	public function get_captured_payment_note( $entry_id ) {
		...
		if ( rgars( $feed, 'meta/setupFee_enabled' ) ) {
			$note = esc_html__( 'Setup fee has been paid.', 'esignbinding' );
		} else {
			$note = esc_html__( 'Trial has been paid.', 'esignbinding' );
		}
	...
	public function stripe_checkout_redirect_scripts( $entry, $form ) {
		...
		if ( ! empty( $session_id ) && ! empty( $feed ) ) {
			...
			<div class="GF_AJAX_POSTBACK">
				<p class="gform_stripe_checkout_message"><?php esc_html_e( 'You\'re being redirected to the hosted Checkout page on Stripe...', 'esignbinding' ); ?></p>
				<script>
	...
	public function get_payment_description( $entry, $submission_data, $feed ) {
		...
		if ( $entry['id'] ) {
			$strings['entry_id'] = sprintf( esc_html__( 'Entry ID: %d', 'esignbinding' ), $entry['id'] );
		}

		$strings['products'] = sprintf(
			_n( 'Product: %s', 'Products: %s', count( $submission_data['line_items'] ), 'esignbinding' ),
	...
	public function billing_info_fields() {
		$fields = array(
			...
			array(
				'name'       => 'address_line2',
				'label'      => __( 'Address 2', 'esignbinding' ),
				'required'   => false,
				'field_type' => array( 'address' ),
			),
			array(
				'name'       => 'address_city',
				'label'      => __( 'City', 'esignbinding' ),
				'required'   => false,
				'field_type' => array( 'address' ),
			),
			array(
				'name'       => 'address_state',
				'label'      => __( 'State', 'esignbinding' ),
				'required'   => false,
				'field_type' => array( 'address' ),
			),
			array(
				'name'       => 'address_zip',
				'label'      => __( 'Zip', 'esignbinding' ),
				'required'   => false,
				'field_type' => array( 'address' ),
			),
			array(
				'name'       => 'address_country',
				'label'      => __( 'Country', 'esignbinding' ),
				'required'   => false,
				'field_type' => array( 'address' ),
			),
		);
	...
	public function handle_upgrade_3_3_3( $previous_version ) {
		...
		if ( $checkout_method === 'stripe_elements' ) {
			...
			$message = sprintf(
				esc_html__( '%1$sYour Gravity Forms Stripe Add-On has been updated to 3.4, and now supports Strong Customer Authentication (SCA/PSD2).%2$s%3$sRefer to %4$sthis guide%5$s for more information on payment methods and SCA.%6$s', 'esignbinding' ),
				'<p>',
				'</p>',
				'<p>',
				'<a href="https://docs.gravityforms.com/stripe-support-of-strong-customer-authentication/" target="_blank">',
			...
			$message = sprintf(
				esc_html__( '%1$sYour Gravity Forms Stripe Add-On has been updated to 3.4, and it no longer supports the Gravity Forms Credit Card Field in new forms (current integrations can still work as usual).%2$s%3$sRefer to %4$sthis guide%5$s for more information about this change.%6$s', 'esignbinding' ),
				'<p>',
				'</p>',
				'<p>',
	...
	public function create_payment_intent() {
		...
		if ( $intent->status === 'requires_confirmation' ) {
			...
			$error = sprintf( esc_html__( 'PaymentIntent status: %s is invalid.', 'esignbinding' ), $intent->status );
	...
	public function ajax_capture_payment() {
		...
		$entry = GFAPI::get_entry( $entry_id );
		if ( is_wp_error( $entry ) ) {
			wp_send_json_error( esc_html__( 'Unable to find entry.', 'esignbinding' ) );
		}
		...
		$feed = $this->get_payment_feed( $entry, $form );
		if ( is_wp_error( $feed ) ) {
			wp_send_json_error( esc_html__( 'Unable to find payment feed.', 'esignbinding' ) );
		}

		// Including Stripe API.
		$result = $this->include_stripe_api_for_entry( $entry_id );
		if ( is_wp_error( $result ) ) {
			wp_send_json_error( esc_html__( 'Could not initialize Stripe API.', 'esignbinding' ) . ' ' . $result->get_error_message() );
			return;
		}
	...
	public function ajax_refund() {
		...
		$entry    = GFAPI::get_entry( $entry_id );
		if ( is_wp_error( $entry ) ) {
			wp_send_json_error( array( 'message' => __( 'Unable to find entry.', 'esignbinding' ) ) );
		}
		...
		$feed = $this->get_payment_feed( $entry, $form );
		if ( is_wp_error( $feed ) ) {
			wp_send_json_error( array( 'message' => __( 'Unable to find payment feed.', 'esignbinding' ) ) );
		}
		...
		if ( ! $payment ) {
			wp_send_json_error( array( 'message' => __( 'Unable to find payment on Stripe', 'esignbinding' ) ) );
		}
	...
	public function maybe_hit_rate_limits( $form_id ) {
		...
		if ( $error_count >= $max_error_count ) {
			...
			return array(
				'error_message' => esc_html__( 'We are not able to process your payment request at the moment. Please try again later.', 'esignbinding' ),
				'is_success'    => false,
				'is_authorized' => false,
			);
		}
	...
	private function get_stripe_display_name( $stripe_account ) {
		...
		return ! empty( $display_name ) ? $display_name : esc_html__( 'Unnamed account', 'esignbinding' );
	}
	...
	private function capture_payment( $transaction_id ) {
		...
		if ( is_wp_error( $payment_object ) ) {
			...
			return array(
				'is_captured' => false,
				'message'     => esc_html__( 'Could not retrieve payment information from Stripe.', 'esignbinding' ),
			);
		}
		...
		if ( $this->is_payment_captured( $payment_object ) ) {
			return array(
				'is_captured'     => true,
				'message'         => esc_html__( 'Payment has already been captured.', 'esignbinding' ),
		...
		if ( is_wp_error( $captured_payment_object ) || ! $this->is_payment_captured( $captured_payment_object ) ) {
			...
			$wp_error = is_wp_error( $captured_payment_object ) ? $captured_payment_object : new WP_Error( 'could_not_capture', esc_html__( 'Unable to capture payment', 'esignbinding' ) );
		...
		return array(
			'is_captured'    => true,
			'message'        => esc_html__( 'Payment captured successfully.', 'esignbinding' ),
	...
	private function include_stripe_api_for_entry( $entry_id ) {
		...
		if ( empty( $feed_ids ) ) {
			...
			return new WP_Error( 'feed_not_found', esc_html__( 'Feed associated with entry could not be found.', 'esignbinding' ) );
		}
		$feed_id = $feed_ids[0];
	...
	public function maybe_display_refund_button( $form_id, $entry ) {
		...
		printf(
			...
			esc_attr( $entry['id'] ),
			esc_html__( 'Refund Payment', 'esignbinding' ),
			esc_html__( 'Sending refund request', 'esignbinding' )
		);
	}
