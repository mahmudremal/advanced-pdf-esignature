...
class GF_FlutterWave_Credit_Card_Field extends \GF_Field {
    ...
    public function get_form_editor_field_title() {
        return esc_attr__('FlutterWave', 'esignbinding');
    }

    public function get_form_editor_field_icon() {
        $icon = file_get_contents(ESIGNBINDING_ADDONS_BUILD_PATH . '/icons/money-business-and-finance-svgrepo-com.svg');
        $icon = str_replace(['width="800px"', 'height="800px"'], ['width="20px"', 'height="20px"'], $icon);
        return $icon;
	}
    public function get_form_editor_field_description() {
		return esc_attr__( 'Secure payments though Flutterwave payment gateway.', 'esignbinding' );
	}
    ...
    public function get_field_input($form, $value = '', $entry = null) {
        ...
        <div class="">
            <?php if($this->enableCardPaymentMethod): ?>
                <!-- <label class="form-label"><?php esc_html_e('Payment Method', 'esignbinding'); ?></label> -->
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="flutterwave-credit"><?php esc_html_e('Credit Card', 'esignbinding'); ?></label>
                </div>
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="flutterwave-checkout"><?php esc_html_e('Flutterwave', 'esignbinding'); ?></label>
                </div>
            <?php endif; ?>
        </div>
        <?php if($this->enableCardPaymentMethod): ?>
            <div class="flutterwaves_credit_card" style="<?php echo esc_attr(($this->flutterwaveDefaultModeCard == true)?'display: flex;':'display: none;'); ?>">
                <div class="creditcard_container preload <?php echo esc_attr(($this->enablePreviewField)?'':'d-none'); ?>">
                    <div class="creditcard">
                        <div class="front">
                            <div id="ccsingle"></div>
                            <?php
                                $icon = ESIGNBINDING_ADDONS_BUILD_PATH . '/icons/sample-credit-card-front.svg';
                        ...
                        <div class="back">
                            <?php
                                $icon = ESIGNBINDING_ADDONS_BUILD_PATH . '/icons/sample-credit-card-back.svg';
                ...
                <div class="form-container">
                    <div class="field-container">
                        <label for="name"><?php esc_html_e('Name', 'esignbinding'); ?></label>
                        <input id="name" maxlength="20" type="text" name="input_<?php echo esc_attr($this->id).'.5'; ?>" data-name="name" required>
                    </div>
                    <div class="field-container">
                        <label for="cardnumber"><?php esc_html_e('Card Number', 'esignbinding'); ?></label>
                        <span id="generatecard" class="<?php echo esc_attr(ESIGNBINDING_ADDONS_TEST_MODE?'':'d-none'); ?>"><?php esc_html_e('generate random', 'esignbinding'); ?></span>
                    ...
                    <div class="field-container">
                        <label for="expirationdate"><?php esc_html_e('Expiration (mm/yy)', 'esignbinding'); ?></label>
                    ...
                    <div class="field-container">
                        <label for="securitycode"><?php esc_html_e('Security Code', 'esignbinding'); ?></label>
                    ...
                    <div class="field-container my-3">
                        ...
                        <button id="submitFlutterCard" type="button">
                            <?php echo esc_html(empty($this->submitBtnText)?__('Continue to pay', 'esignbinding'):$this->submitBtnText); ?>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="card p-2">
                <p>
                    <?php echo esc_html(($this->enableCardPaymentMethod)?(
                        empty($this->fluttercardMessage)?__('Upon selecting the payment method as "Flutterwave," you will be redirected to a secure payment link. Please note that your form submission will remain pending until you complete the payment process successfully.', 'esignbinding'):$this->fluttercardMessage
                        ):(
                            empty($this->fluttercardMessage)?__('You will be redirected to a secure payment link. Please note that your form submission will remain pending until you complete the payment process successfully.', 'esignbinding'):$this->fluttercardMessage
                        )); ?>
                </p>
            </div>
        </div>
    ...
    public function validate($value, $form) {
        ...
        if (isset($form['fields'][$flutterwaveFieldId])) {
            ...
            if (rgblank($flutterwaveFieldValue)) {
                ...
                $form['fields'][$flutterwaveFieldId]->validation_message = esc_html__('Flutterwave Payment: This field is required. Please fix it.', 'esignbinding');
            } else {
    ...
    public function is_flutterwave_enabled() {
        $settings = ESIGNBINDING_ADDONS_OPTIONS;
			...
			if($this->is_flutterwave_enabled()) {
                ...
                <div>
                    ...
                    <label for="field_enable_card_payment_method" class="inline">
                        <?php esc_html_e( 'Enable multiple payment methods', 'esignbinding' ); ?>
                        <?php gform_tooltip( 'form_field_enable_card_payment_method' ); ?>
                    </label>
                    <div id="field_multiple_payment_methods_description">
                        <?php
                        // translators: variables are the markup to generate a link.
                        printf( esc_html__( 'Available payment methods can be configured in your %1$sFlutterwave Dashboard%2$s.', 'esignbinding' ), '<a href="https://dashboard.flutterwave.com/" target="_blank">', '</a>' );
                        ?>
                    </div>
                </div>
                <br>
