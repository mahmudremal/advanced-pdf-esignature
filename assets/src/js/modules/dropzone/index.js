   ...
   */
    ...
    videoDurationValidate( file ) {
      ...
      if ( fileSize > thisClass.videoControl.uploadMaxSize ) {
        toast.show({title: thisClass.__( 'Filesize exceed it maximum limit 30MB.', 'esignbinding' ), position: 'bottomright', type: 'warn'});
        return false;
      } else {
        reader.onload = function(e) {
          if (file.type == "video/mp4" || file.type == "video/ogg" || file.type == "video/webm") {
            ...
            var timer = setInterval(function() {
              if (videoElement.readyState === 4) {
                ...
                if (getTime > thisClass.videoControl.videoMaxTime ) {
                  toast.show({title: thisClass.__( 'Video exceed it\'s duration limit.', 'esignbinding' ) + thisClass.videoControl.videoMaxTime, position: 'bottomright', type: 'warn'});
                  return false;
                } else {
                  return true;
                }
            ...
            var timer = setInterval(function() {
              if (audioElement.readyState === 4) {
                ...
                if (getTime > thisClass.videoControl.audioMaxTime ) {
                  toast.show({title: thisClass.__( 'Audio exceed it\'s duration limit.', 'esignbinding' ) + thisClass.videoControl.audioMaxTime, position: 'bottomright', type: 'warn'});
                  return false;
                } else {
                  return true;
                }
            ...
            var timer = setInterval(function() {
              if (file) {
                toast.show({title: thisClass.__( 'Invalid file formate.', 'esignbinding' ) + file.type, position: 'bottomright', type: 'warn'});
                clearInterval(timer);
                return false;
              }
            }, 500)
