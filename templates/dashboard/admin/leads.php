        ...
        <div class="card-body">
          <div class="fancy-table table-responsive border rounded">
            <table class="table table-striped mb-0">
              <thead>
                <tr>
                  <th scope="col"><?php esc_html_e( 'Email', 'esignbinding' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Profiles', 'esignbinding' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Client Status', 'esignbinding' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Meeting Time & link', 'esignbinding' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Retainer Amount', 'esignbinding' ); ?></th>
                  <!-- Contract Options -->
                  <th scope="col"><?php esc_html_e( 'Status', 'esignbinding' ); ?></th>
                  <th scope="col"><?php esc_html_e( 'Action', 'esignbinding' ); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php if( count( $subscribers ) <= 0 ) : ?>
                  <tr>
                    <td colspan="7"><img src="<?php echo esc_url( ESIGNBINDING_ADDONS_BUILD_URI . '/icons/not-found.svg' ); ?>" alt="<?php esc_attr_e( 'Noting found', 'esignbinding' ); ?>" style="width: 100%;height: auto;max-height: 400px;"></td>
                  </tr>
                <?php endif; ?>
                <?php foreach( $subscribers as $subscriber ) :
                  ...
                  $userInfo = (object) wp_parse_args( $userInfo, [
                    'id'      => '',
                    'meta'      => (object) wp_parse_args( $userMeta, apply_filters( 'esign/project/usermeta/defaults', (array) $userMeta ) )
                  ] );
                  $activeSubscription = in_array( $userInfo->meta->enable_subscription, [ 'on' ] );
                  $doc = apply_filters( 'esign/project/esign/userdocument', false, $userInfo );$doc = ( $doc && is_array( $doc ) ) ? (object) $doc: $doc;
                  if( $doc && ! empty( $doc->document_status ) ) {$is_signed = ( $doc->document_status == 'signed' );} else {$is_signed = null;}
                    //   Missing
                  $is_signed = ( $is_signed === null ) ? __( 'Pending', 'esignbinding' ) : (
                    ( $is_signed === true ) ? __( 'Signed', 'esignbinding' ) : __( 'Pending', 'esignbinding' )
                  );
                  $is_active = $activeSubscription ? __( 'Active', 'esignbinding' ) : __( 'Paused', 'esignbinding' );
                  ...
                  <tr id="lead-<?php echo esc_html( $userInfo->ID ); ?>">
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        ...
                        <?php $flag = apply_filters( 'esign/project/database/countryflags', get_user_meta( $userInfo->ID, 'country', true ) );if( $flag ) : ?>
                    ...
                    <td id="lead-status-<?php echo esc_html( $userInfo->ID ); ?>">
                      <?php $status = apply_filters( 'esign/project/action/statuses', [], $userInfo->meta->status ); ?>
                      <?php $status = is_string( $status ) ? $status : ''; ?>
                      <span class="badge bg-soft-success p-2 text-success"><?php echo esc_html( $status ); ?></span>
                      <?php // echo apply_filters( 'esign/project/widgets/statustab', '', $userInfo ); ?>
                    ...
                    <td>
                      <div class="d-flex align-items-center">
                        <div class="media-support-info">
                          ...
                          <a class="mb-0" href="<?php echo esc_url( $userInfo->meta->meeting_link ); ?>" target="_blank">@<?php esc_html_e( 'Meeting link', 'esignbinding' ); ?></a>
                          <?php endif; ?>
                        </div>
                      </div>
                    </td>
                    ...
                    <td>
                      <div class="d-flex justify-content-evenly">
                        <a class="btn btn-primary btn-icon btn-sm rounded-pill" href="<?php echo esc_url( apply_filters( 'esign/project/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename ) ); ?>" role="button" target="_blank">
