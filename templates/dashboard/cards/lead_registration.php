...
if( is_user_logged_in() ) {
  $user_slug = apply_filters( 'esign/project/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename );
  wp_redirect( $user_slug );exit;
}
// $_SESSION[ 'current-lead' ] = $userInfo->ID;
if( get_transient( '_lead_user_registration-' . apply_filters( 'esign/project/user/visitorip', '' ) ) ) {
  delete_transient( '_lead_user_registration-' . apply_filters( 'esign/project/user/visitorip', '' ) );
}
set_transient( '_lead_user_registration-' . apply_filters( 'esign/project/user/visitorip', '' ), $userInfo->ID, 7200 );
...
if( $is_done && $is_done >= 10 ) {
  wp_redirect( apply_filters( 'esign/project/user/dashboardpermalink', $userInfo->ID, $userInfo->data->user_nicename ) );exit;
}
$needToSelect = false;
...
if( ! $regLink || empty( $regLink ) || (int) $regLink <= 0 ) {
   ...
   * Woonna code here.
   */
  $contractForms = apply_filters( 'esign/project/action/contractforms', [], false );
...
$regLink = apply_filters( 'esign/project/system/getoption', 'regis-link-url-' . $regLink, false );
...
if( $regLink && ! empty( $regLink ) && ! $needToSelect ) {
  ...
  $defaultContract = apply_filters( 'esign/project/system/getoption', 'signature-defaultcontract', '' );
  if( ! empty( $defaultContract ) ) {
    wp_redirect( $defaultContract );exit;
  } else {
    wp_die( __( 'Regisatration link not found.', 'esignbinding' ) );
  }
}
...
if( is_wp_error( $userInfo ) || $errorHappens ) :
  ...
  add_filter( 'pre_get_document_title', function( $title ) {
    $title = apply_filters( 'esign/project/system/getoption', 'dashboard-title', __( 'Registration Field', 'esignbinding' ) );
    return $title;
  }, 10, 1 );
  get_header();
  ?>
...
<?php if( true ) : ?>
  <?php include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/cards/select_registration_page.php'; ?>
<?php else: ?>
          ...
          <div class="content fs-6 d-flex flex-column-fluid">
            <div class="container-xxl">
              <div class="row g-5 g-xxl-12">
                <div class="col-xl-12">
                  <?php // include ESIGNBINDING_ADDONS_DIR_PATH . '/templates/dashboard/cards/form-wizard.php'; ?>
                </div>
              </div>
            </div>
          </div>
