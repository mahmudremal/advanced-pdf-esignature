...
if($entry && !is_wp_error($entry) && isset($entry['source_url']) && !empty($entry['source_url']) && $form && !is_wp_error($form)) {
    foreach($form['fields'] as $i => $field) {
        if($field->type == 'flutterwave_credit_card' && $field->statusBtnLink && $field->statusBtnLink == 'form') {
            $backtoLink = $entry['source_url'];$backtoText = __('Back to form', 'esignbinding');
        }
    }
}
...
if(in_array($payment_status, ['success', 'successful'])) {
    if($verify) {
        ...
        wp_die(
            sprintf(
                __('We can\'t verify this transaction. Please contact with support. Transaction no is. %s', 'esignbinding'),
                '<b>'.$transaction_id.'</b>'
            ), __('Verification failed!', 'esignbinding')
        );
    }
}
...
<?php if($payment_status): ?>
        ...
        <section class="fltrwv__payment-status-content overflow-hidden container">
            <div class="fltrwv__row no-gutters align-items-center fltrwv__bg-white my-5">      
                            ...
                            <div class="fltrwv__card fltrwv__d-flex fltrwv__justify-content-center mb-0">
                                <div class="fltrwv__card-body">
                                    <h2 class="fltrwv__mt-3 fltrwv__mb-4">
                                        <?php echo esc_html(in_array($payment_status, ['success','successful'])?__('Payment Successful', 'esignbinding'):__( 'Payment Failed',   'esignbinding')); ?>
                                    </h2>
                ...
                <div class="fltrwv__col-lg-6 fltrwv__d-lg-block fltrwv__p-0 overflow-hidden">
                    <img alt="images" loading="lazy" data-src="<?php echo esc_url(ESIGNBINDING_ADDONS_BUILD_URI.'/icons/Card Payment_Monochromatic.svg'); ?>" class="fltrwv__img-fluid fltrwv__gradient-main lazyloaded" src="<?php echo esc_url(ESIGNBINDING_ADDONS_BUILD_URI.'/icons/Card Payment_Monochromatic.svg'); ?>">
                    <noscript>
                        <img src="<?php echo esc_url(ESIGNBINDING_ADDONS_BUILD_URI.'/icons/Card Payment_Monochromatic.svg'); ?>" class="fltrwv__img-fluid fltrwv__gradient-main" alt="images" loading="lazy">
                    </noscript>
                </div>
            </div>
        </section>
  ...
  <?php get_footer(); ?>
    <?php else:
  wp_die(__('Something suspicious detected or it would be probably Failed your payment. Please try again.', 'esignbinding'));
  ?>
<?php endif; ?>