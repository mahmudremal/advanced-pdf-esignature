...
if( $userInfo ) {
  ...
  $payment_link = ( empty( $userInfo->meta->monthly_retainer ) || $userInfo->meta->monthly_retainer <= 0 ) ? false : apply_filters( 'gravityformsflutterwaveaddons/project/payment/stripe/paymentlink', [
    'quantity'	=> 1,
    'price_data' => [
      'currency' => apply_filters( 'gravityformsflutterwaveaddons/project/system/getoption', 'stripe-currency', 'usd' ),
      'unit_amount' => (int) ( $userInfo->meta->monthly_retainer * 100 ), // Unit amount in cent | number_format( $userInfo->meta->monthly_retainer, 2 ),
      'product_data' => [
        'name' => apply_filters( 'gravityformsflutterwaveaddons/project/system/getoption', 'stripe-productname', __( 'Subscription',   'esignbinding' ) ),
        'description' => apply_filters( 'gravityformsflutterwaveaddons/project/system/getoption', 'stripe-productdesc', __( 'Payment for',   'esignbinding' ) . ' ' . get_option( 'blogname', 'We Make Content' ) ),
        'images' => [ apply_filters( 'gravityformsflutterwaveaddons/project/system/getoption', 'stripe-productimg', esc_url( ESIGNBINDING_ADDONS_BUILD_URI . '/icons/Online payment_Flatline.svg' ) ) ],
      ],
    ]
  ], true );
